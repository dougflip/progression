...
1. **Done.** Fixed `isDirty()` to also report dirty when there's preset-worthy state (sections with loops) but no preset is loaded, instead of early-returning `false` with nothing to diff against.
2. **Done.** Added a `beforeunload` guard (`app.ts`) gated on that fixed dirty state — a native "leave site? unsaved changes" browser prompt now fires at the exact moment (refresh/close/navigate) the recording would otherwise be lost, which a passive UI indicator can't do since you're not looking at it in that moment.
3. Autosave draft slot — persist live `AppState` (loops included) to its own localStorage key, independent of named presets, merged back in on boot the same way `mergeSectionLoops` already does for a loaded preset. Closes the gap completely, even if the user never hits Save (the `beforeunload` guard above only helps if they notice and cancel the prompt), but it's a second persistence channel with the same boot-ordering trap 2b already hit once (`mergeSectionLoops` exists because of that bug), so it needs care.
