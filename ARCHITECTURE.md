...
- **Readout pill flyouts (style, tempo, bars, voicing, loop) share one shell and one positioning routine.** Each is a `.pill-flyout` div, opened/closed via a shared `wireFlyout(triggerBtn, flyoutEl, onOpen?)` (toggle + outside-click dismiss). `positionFlyout` runs on every open: it anchors the flyout's left or right edge to `#readout`'s corresponding edge, picking the side based on whether the trigger pill's center falls in the left or right half of `window.innerWidth` — a pill near either screen edge (e.g. the leftmost Style pill) can't push its flyout off-screen. The arrow is positioned separately via `--arrow-left`, computed after anchoring so it always points at the real trigger pill regardless of which edge the box anchored to. Only Tempo keeps bespoke content (slider + tap button) inside the shared shell; Style/Bars/Voicing/Loop render a plain option list via `renderOptionRows`/`makePickerRow`.
...
**Invariant: `key` must equal `customCycleKeys[0]` whenever `cycle === 'custom'`.** Every resolved key is `key` shifted by a `getShiftsForCycle` offset, and custom-cycle offsets are computed relative to `customCycleKeys[0]` — so if `key` ever drifts from `customCycleKeys[0]` (e.g. a shared URL with `key=C&cycle=custom&customKeys=A,D,G`, or reordering the sequence so a different key lands at index 0), every lap resolves against the wrong home key. `normalizePlaybackForCycle(playback)` enforces this by syncing `key` to `customCycleKeys[0]` whenever both are present, and is called everywhere a `PlaybackSettings` object gets constructed from a partial: `parseUrl`, `_setPlayback`, `_applyPresetState`, and `setLoadedBuiltinPresetContext`.
